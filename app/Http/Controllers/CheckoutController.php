...
{
    public function handleVNPayURLIPN(Request $request) {
        dd($request);
    }
    public function handleVNPayReturn(Request $request) {
        ...
        if($data["vnp_ResponseCode"] === '00') {
            ...
            return redirect()->route('payment.result.view')->with('message', $message);
        }
    ...
    public function vnpayPayment(Request $request)
    {
        ...
        $vnp_TmnCode = "DT8YK49T"; //Mã website tại VNPAY 
        $vnp_HashSecret = "XTYSVQFPQYMXNLHWLLGGLMYOGGAOTOQH"; //Chuỗi bí mật

        $vnp_TxnRef = rand(00000, 99999); //Mã đơn hàng. Trong thực tế Merchant cần insert đơn hàng vào DB và gửi mã này sang VNPAY
        $vnp_OrderInfo = 'Payment order test';
